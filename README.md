...
- [x] Explain what a token is used for.
      ...
      A token is used like a key to control access to a private routes on web apps    


- [x] What steps can you take in your web apps to keep your data secure?

      We can require that users access sensitive areas of our web apps via a protected route that requires authentication. 

- [x] Describe how web servers work.

      “Web server” can refer to two different things:
        1) A physical computer that is connected to the internet, and stores the code for different websites to be shared across the world at all times is known as a web server. This physical server is “hosting” our website.
        2) The software that runs the physical server is also a “web server”. This software retrieves our code and serves  it when requested by the user
...
### API Documentation
  ...
  * **[POST]** * to `/api/login`: returns a token to be added to the header of all other requests. Pass in the following credentials as the `body` of the request: `{ username: 'name', password: 'pwd' }`
